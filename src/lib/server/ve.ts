...
function from_payload(p: Record<string, unknown>): Ve | null {
	...
	return { s: 'e', i: p.i as string, u: p.u as string, p: p.p as string, m: p.m as string, g: p.g as number | undefined, z: p.z as string | undefined, r: p.r as number, t: p.t as number, c: p.c as string | undefined, l: p.l as number | undefined, j: p.j as string | undefined, w: p.w as string | undefined, d: p.d as number };
}

export async function save_ve(id: string, u: string, prompt: string, model: string, period: number, duration?: number, resolution?: string, job_id?: string): Promise<void> {
	const v: Ve = { s: 'e', i: id, u, p: prompt, m: model, g: duration, z: resolution, r: period, t: 0, c: job_id ? 'sampling' : undefined, j: job_id, d: Date.now() };
